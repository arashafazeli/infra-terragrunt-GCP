...
locals {
  ...
  actual_availability_type      = local.is_postgres && var.db_enable_failover_replica ? "REGIONAL" : "ZONAL"
  actual_failover_replica_count = local.is_postgres ? 0 : var.db_enable_failover_replica ? 1 : 0
}
...
resource "google_sql_database_instance" "master" {
  ...
  settings {
    ...
    insights_config {
      query_insights_enabled  = var.db_insights_enabled
      query_string_length     = var.db_insights_query_string_length
      record_application_tags = var.db_insights_record_application_tags
      record_client_address   = var.db_insights_record_client_address
    }
