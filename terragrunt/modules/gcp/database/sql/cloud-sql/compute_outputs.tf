...
data "template_file" "failover_proxy_connection" {
  ...
  template = "${data.google_client_config.current.project}:${data.google_client_config.current.region}:${google_sql_database_instance.failover_replica.0.name}"
}
...
data "template_file" "read_replica_proxy_connection" {
  ...
  template = "${data.google_client_config.current.project}:${data.google_client_config.current.region}:${google_sql_database_instance.read_replica.*.name[count.index]}"
}
