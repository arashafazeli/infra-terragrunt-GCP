...
output "client_certificate" {
  ...
  value       = base64decode(google_container_cluster.autopilot.master_auth[0].client_certificate)
}

output "client_key" {
  description = "Private key used by clients to authenticate to the cluster endpoint."
  value       = base64decode(google_container_cluster.autopilot.master_auth[0].client_key)
  sensitive   = true
}
