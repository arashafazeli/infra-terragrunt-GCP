...
dependency "global-dev-dns-zone" {
  config_path = "${get_parent_terragrunt_dir()}/tech/dev/global/network/dns/global.dev.gloot.com/dns-zone"
  mock_outputs = {
    nameservers = ["invalid"]
  }
  mock_outputs_allowed_terraform_commands = ["plan", "destroy"]
...
inputs = {
  ...
  type         = "NS"
  ttl          = 21600
  rrdatas      = dependency.global-dev-dns-zone.outputs.nameservers
